if game.Players.LocalPlayer.PlayerGui:FindFirstChild("ZayrosFISHIT") ~= nil then
	...
	-- Floating Icon Buttons (FAB Dock)
	local FloatingDock = Instance.new("Frame")
	FloatingDock.Name = "FloatingDock"
	FloatingDock.Parent = FrameUtama
	FloatingDock.AnchorPoint = Vector2.new(1, 1)
	FloatingDock.BackgroundTransparency = 1
	FloatingDock.Position = UDim2.new(0.985, 0, 0.965, 0)
	FloatingDock.Size = UDim2.new(0, 60, 0, 280)
	FloatingDock.ZIndex = 10

	local DockLayout = Instance.new("UIListLayout")
	DockLayout.Parent = FloatingDock
	...
	local function createFabButton(name, imageOffset)
		local btn = Instance.new("ImageButton")
		btn.Name = name
		btn.Parent = FloatingDock
		btn.Size = UDim2.new(0, 50, 0, 50)
		btn.BackgroundColor3 = Color3.fromRGB(32, 32, 32)
		btn.AutoButtonColor = true
		btn.ZIndex = 11
		btn.Image = "rbxassetid://3926305904" -- Roblox UI icons sprite sheet
		btn.ImageColor3 = Color3.fromRGB(255, 255, 255)
		btn.ImageRectSize = Vector2.new(36, 36)
		btn.ImageRectOffset = imageOffset or Vector2.new(0, 0)
	...
	-- Create FABs (icon offsets chosen from the sprite sheet)
	local FabTeleport = createFabButton("FabTeleport", Vector2.new(324, 324)) -- map/location icon
	local FabBoat = createFabButton("FabBoat", Vector2.new(648, 324)) -- vehicle/boat-ish
	local FabPlayer = createFabButton("FabPlayer", Vector2.new(324, 84)) -- user icon
	local FabMain = createFabButton("FabMain", Vector2.new(204, 204)) -- home icon
	local FabSettings = createFabButton("FabSettings", Vector2.new(84, 204)) -- gear icon

	-- Hide legacy sidebar menu
	...
	-- Tambah handler untuk FAB agar membuka panel sesuai ikon
	if FabMain then
		FabMain.MouseButton1Click:Connect(function()
			showPanel("Main")
		end)
	end
	if FabPlayer then
		FabPlayer.MouseButton1Click:Connect(function()
			showPanel("Player")
		end)
	end
	if FabTeleport then
		FabTeleport.MouseButton1Click:Connect(function()
			showPanel("Teleport")
		end)
	end
	if FabBoat then
		FabBoat.MouseButton1Click:Connect(function()
			showPanel("Boat")
		end)
	end
	if FabSettings then
		FabSettings.MouseButton1Click:Connect(function()
			-- Placeholder settings action. Dapat diisi nanti.
		end)
	end
